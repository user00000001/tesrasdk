...
//tesra sdk in golang. Using for operation with Tesra
package tesra_go_sdk

import (
	"encoding/hex"
	"fmt"
	"github.com/TesraSupernet/go-bip32"
	"github.com/TesraSupernet/tesrasdk/bip44"
	...
	"github.com/TesraSupernet/tesrasdk/client"
	common3 "github.com/TesraSupernet/tesrasdk/common"
...
//TesraSdk is the main struct for user
type TesraSdk struct {
	client.ClientMgr
...
//NewTesraSdk return TesraSdk.
func NewTesraSdk() *TesraSdk {
	tesraSdk := &TesraSdk{}
	native := newNativeContract(tesraSdk)
	tesraSdk.Native = native
	teoVM := newTeoVMContract(tesraSdk)
	tesraSdk.TeoVM = teoVM
	wasmVM := newWasmVMContract(tesraSdk)
	tesraSdk.WasmVM = wasmVM
	return tesraSdk
}

//CreateWallet return a new wallet
func (this *TesraSdk) CreateWallet(walletFile string) (*Wallet, error) {
...
func (this *TesraSdk) OpenWallet(walletFile string) (*Wallet, error) {
...
func ParsePayload(code []byte) (map[string]interface{}, error) {
	l := len(code)
	if l > 44 && string(code[l-22:]) == "TesraSupernet.Native.Invoke" {
		//46 = 22  "TesraSupernet.Native.Invoke"
		// +1   length
		// +1   SYSCALL
		// +1   version
		// +20  address
		...
		if l > 54 && string(code[l-46-8:l-46]) == "transfer" {
			...
			res["param"] = param
			if contractAddress == TSR_CONTRACT_ADDRESS {
				res["asset"] = "tsr"
			} else if contractAddress == TSG_CONTRACT_ADDRESS {
				res["asset"] = "tsg"
			}
			return res, nil
...
func getValue(source *common.ZeroCopySource) (uint64, error) {
	...
	if zeroByte == 0 {
		...
		b := common.BigIntFromTeoBytes([]byte{zeroByte})
		...
		amount = common.BigIntFromTeoBytes(amountBytes).Uint64()
	}
	return amount, nil
}
...
func (this *TesraSdk) GenerateMnemonicCodesStr() (string, error) {
...
func (this *TesraSdk) GetPrivateKeyFromMnemonicCodesStrBip44(mnemonicCodesStr string, index uint32) ([]byte, error) {
	if mnemonicCodesStr == "" {
...
func (this *TesraSdk) NewInvokeTransaction(gasPrice, gasLimit uint64, invokeCode []byte) *types.MutableTransaction {
...
func (this *TesraSdk) SetPayer(tx *types.MutableTransaction, payer common.Address) {
	tx.Payer = payer
}

func (this *TesraSdk) SignToTransaction(tx *types.MutableTransaction, signer Signer) error {
...
func (this *TesraSdk) MultiSignToTransaction(tx *types.MutableTransaction, m uint16, pubKeys []keypair.PublicKey, signer Signer) error {
	pkSize := len(pubKeys)
...
func (this *TesraSdk) GetTxData(tx *types.MutableTransaction) (string, error) {
...
func (this *TesraSdk) ParseNaitveTransferEvent(event *event.NotifyEventInfo) (*TransferEvent, error) {
	if event == nil {
...
func (this *TesraSdk) GetMutableTx(rawTx string) (*types.MutableTransaction, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("[TSR]IntoMutable error:%s", err)
	}
	return mutTx, nil
}

func (this *TesraSdk) GetMultiAddr(pubkeys []keypair.PublicKey, m int) (string, error) {
...
func (this *TesraSdk) GetAdddrByPubKey(pubKey keypair.PublicKey) string {
	address := types.AddressFromPubKey(pubKey)
	return address.ToBase58()
}

func (this *TesraSdk) SendTransaction(tx *types.MutableTransaction) (common.Uint256, error) {
